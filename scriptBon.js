...
const createTaskElement = (text, isDone = false, id) => {
...
let firebaseTasksCache = [];

const renderTasks = () => {
  taskList.innerHTML = "";

  firebaseTasksCache.forEach(task => {
    if (
      currentFilter === "All" ||
      (currentFilter === "Done" && task.isDone) ||
      (currentFilter === "Todo" && !task.isDone)
    ) {
      createTaskElement(task.text, task.isDone, task.id);
    }
  });
...
onValue(ref(db, "tasks"), (snapshot) => {
  ...
  firebaseTasksCache = Object.entries(data).map(([id, task]) => ({
    id,
    ...task
  }));
  renderTasks();
